...
```{r}
ggplot(df_st_latest[TotalCases>5000], aes(x=reorder(USAState, -(TotalDeaths/TotalCases)), y=TotalDeaths/TotalCases, fill="CFR"))+geom_col()+ggtitle("US COVID-19 Tot.Deaths/Tot.cases by state")+ theme(axis.text.x = element_text(angle = 90))+ guides(fill=guide_legend(title=NULL))+xlab("State")

```

```{r}
ggplot(df_st_latest[TotalCases>5000], aes(x=reorder(USAState, -(TotalCases/TotalTests)), y=TotalCases/TotalTests, fill="Tot.Cases /Total Tests"))+geom_col()+ggtitle("US COVID-19 Tot. Cases/Tot. tests by state")+ theme(axis.text.x = element_text(angle = 90))+ guides(fill=guide_legend(title=NULL))+xlab("State")
